...
_sam_predictor = None
SAM_MODEL_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "models", "sam_vit_b.pth")

# Known best.pt locations (checked in order)
_BEST_PT_SEARCH = [
    os.path.join(os.path.dirname(os.path.dirname(__file__)), "models", "best.pt"),
    "/mnt/c/Users/salbe/Downloads/FiltrexAIDemo/runs/detect/projects/svs_plumbing/experiments/exp_001_20260318_161743/train/weights/best.pt",
    "/mnt/c/Users/salbe/Downloads/best.pt",
]
...
class AnnotationTool:
    def __init__(self, root, images_dir, json_path, class_names):
        self.root = root
    ...
    def _ensure_image_record(self, filename, w, h):
        rec = self._get_image_record(filename)
        if rec is None:
            new_id = max((img["id"] for img in self.coco["images"]), default=0) + 1
        ...
        for i, ann in enumerate(anns):
            ...
            if seg and seg[0] and len(seg[0]) >= 6:
                ...
                for j in range(0, len(pts), 2):
                    cx, cy = self._img_to_canvas(pts[j], pts[j + 1])
    ...
    def _sam_ensure_embedded(self):
        ...
        if predictor is None:
            if not os.path.exists(SAM_MODEL_PATH):
                self._set_status("SAM model not found — download models/sam_vit_b.pth first.")
            else:
        ...
        if self.sam_embedding_img != current_file:
            self._set_status("SAM embedding image... please wait")
            self.root.update_idletasks()
    ...
    def _on_right_click(self, event):
        # If already in edit mode, handle vertex deletion
        if self.edit_ann_id is not None:
    ...
    def _remove_class(self):
        ...
        if messagebox.askyesno("Remove Class",
                               f"Remove '{name}'?\nAnnotations with this class will NOT be deleted."):
            self.class_names.pop(sel[0])
    ...
    def _batch_pre_annotate(self, conf_threshold=0.35):
        ...
        for i, fname in enumerate(unannotated):
            ...
            # YOLO inference
            try:
                results = model.predict(full_path, conf=conf_threshold, verbose=False)[0]
            except Exception:
                total_failed += 1
                continue
...
def main():
    ...
    if args.project:
        ...
        json_path = args.json
        # Try to load classes from existing JSON
        class_names = []
        if os.path.exists(json_path):
            with open(json_path) as f:
                coco = json.load(f)
